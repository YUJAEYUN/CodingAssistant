#!/usr/bin/env python3
"""
수동 캡처 전용 웹 기반 코딩 어시스턴트

권한 문제 없이 수동 캡처만으로 작동하는 간단한 버전입니다.
"""
...
class ManualWebAssistant:
    ...
    def __init__(self):
        ...
        self.app.config['SECRET_KEY'] = 'manual_assistant_2024'
        ...
        # 초기 메시지
        self.add_message("🤖 안녕하세요! 수동 캡처 코딩 어시스턴트입니다.", "assistant")
        self.add_message("📸 '화면 캡처' 버튼을 눌러 화면을 분석해보세요!", "assistant")
        self.add_message("💬 또는 메시지를 입력하면 화면을 캡처하여 분석합니다.", "assistant")
    ...
    def setup_routes(self):
        ...
        def capture_screen():
            try:
                ...
                self.add_message("📸 화면 캡처를 시작합니다...", "assistant")
                threading.Thread(
    ...
    def capture_screen(self):
        """화면 캡처"""
        try:
            self.console.print("[yellow]📸 화면 캡처 시작...[/yellow]")

            # 화면 캡처
            screenshot = pyautogui.screenshot()

            # 이미지 크기 조정 (5MB 제한을 위해)
            # 원본 크기가 너무 크면 리사이즈
            ...
            if width > max_dimension or height > max_dimension:
                ...
                self.console.print(f"[yellow]📐 이미지 크기 조정: {width}x{height} → {new_width}x{new_height}[/yellow]")
            ...
            self.console.print(f"[green]📸 화면 캡처 완료: {filepath} ({file_size_mb:.1f}MB)[/green]")
            return str(filepath)
    ...
    def analyze_screen_with_ai(self, image_path: str, user_prompt: str) -> str:
        """AI로 화면 분석"""
        try:
            self.console.print("[yellow]🤖 AI 분석 시작...[/yellow]")
...
응답은 간결하고 실용적으로 작성해주세요.
"""
            
            user_message = f"""
사용자 요청: {user_prompt}

첨부된 화면 스크린샷을 분석하고, 사용자의 요청에 맞는 해결책을 제공해주세요.
"""
            ...
            self.console.print("[green]🤖 AI 분석 완료![/green]")
    ...
    def run(self, host='127.0.0.1', port=8080, debug=False):
        """웹 서버 실행"""
        self.console.print(f"[bold cyan]🌐 수동 캡처 코딩 어시스턴트 시작![/bold cyan]")
        self.console.print(f"[green]URL: http://{host}:{port}[/green]")
        self.console.print(f"[yellow]수동 캡처만 지원 - 권한 문제 없음![/yellow]")
